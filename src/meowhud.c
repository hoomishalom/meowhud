...
int main(int argc, char *argv[]) {
  ...
  memset(state.mmapped, 0x80, state.shm_size); // writes all 1s (i.e. not transparent and white)
  ...
  while (state.running) {
    if (state.configured) {
      // memset(state.mmapped, temp, state.shm_size);  // writes all 1s (i.e. not transparent and white)
      char32_t *text = U"Meow!";
      render_chars(text, state.pix_img, 17, 250, state.width, NULL, state.font);
